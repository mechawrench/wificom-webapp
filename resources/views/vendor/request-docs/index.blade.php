...
<head>
    ...
    <meta name="description" content="WiFiCom Request Docs">
...
<nav class="bg-white py-2 ">
    <div class="container px-4 md:flex md:items-center">
        <div class="flex justify-between items-center">
            <a href="{{config('request-docs.url')}}"
               class="font-bold text-xl text-indigo-600">{{ config('request-docs.document_name') }}</a>
        </div>
        <div class="flex justify-between items-center pl-2">
            |
        </div>
        <div class="flex justify-between items-center pl-2">
            <a href="{{config('request-docs.url')}}?openapi=true"
               class="hover:bg-red-600 bg-red-500 text-white pl-5 pr-5 border-transparent shadow-inner border-2 rounded">
    ...
    <aside class="text-sm ml-1.5 text-grey-darkest break-all bg-gray-200 pl-2 h-screen sticky top-1 overflow-auto">
        <section class="pt-5 pl-2 pr-2 pb-5 border mb-10 rounded bg-white shadow">
            <div class="font-sans">
                ...
                <input type="text" v-model="filterTerm" @input="filterDocs"
                       class="w-full p-2 border-2 border-gray-300 rounded" placeholder="/api/search">
            </div>
        </section>
        <section class="pt-5 pl-2 pr-2 pb-5 border mb-10 rounded bg-white shadow">
            <div class="font-sans">
                ...
                <p class="text-xs pb-2 font-medium text-gray-500">Default headers sent on every request. Format <code>Key:Value</code>
                </p>
                <prism-editor
                    class="my-prism-editor"
                    style="max-width:100%;min-height:100px;background:#2d2d2d;color: #ccc;resize:both"
                    v-model="requestHeaders" :highlight="highlighter" line-numbers></prism-editor>
            </div>
        </section>
                        ...
                        <a href="#{{$doc['methods'][0] .'-'. $doc['uri']}}" @click="highlightSidebar({{$index}})"
                           v-if="!docs[{{$index}}]['isHidden']">
                            ...
                            <span class="text-xs" v-bind:class="docs[{{$index}}]['isActiveSidebar'] ? 'font-bold':''">
                                    <span class="text-gray-800 pr-1 pl-1"
                                          v-if="docs[{{$index}}]['responseOk'] === null">{{$doc['uri']}}</span>
                                    <span
                                        class="font-bold text-green-600 border rounded-full pr-1 pl-1 border-green-600"
                                        v-if="docs[{{$index}}]['responseOk'] === true">
                                        {{$doc['uri']}} -
                                        <span
                                    ...
                                    <span class="font-bold text-red-600 border rounded-full pr-1 pl-1 border-red-500"
                                          v-if="docs[{{$index}}]['responseOk'] === false">
                                        {{$doc['uri']}} -
                                        <span
                                            class="inline-flex text-xs"
                                            v-text="'Status:'+docs[{{$index}}]['responseCode'] + ', Took:' + docs[{{$index}}]['responseTime'] + 'ms'">
            ...
            <section class="pt-5 pl-2 pr-2 pb-5 border mb-10 rounded bg-white shadow"
                ...
                <table class="table-fixed text-sm mt-5">
                    ...
                    @foreach ($doc['middlewares'] as $middleware)
                        <tr>
                            <td class="align-left pl-2 pr-2 bg-gray-100 border-r-2">
                                Middleware {{ $loop->index + 1 }}</td>
                ...
                @if (!empty($doc['rules']))
                    <table class="table-fixed align-left text-sm mt-5">
                        ...
                        <tr class="border">
                            ...
                            <th class="border border-gray-300 pl-2 pr-16 pt-1 pb-1 w-min text-left bg-gray-200">
                                Attributes
                            </th>
                            <th class="border border-gray-300 pl-2 pr-16 pt-1 pb-1 w-min text-left bg-gray-200">Type
                            </th>
                            <th class="border border-gray-300 pl-2 pr-16 pt-1 pb-1 w-min text-left bg-gray-200">
                                Nullable
                            </th>
                            <th class="border border-gray-300 pl-2 pr-16 pt-1 pb-1 w-min text-left bg-gray-200">Bail
                            </th>
                            <th class="border border-gray-300 pl-2 pr-16 pt-1 pb-1 w-min text-left bg-gray-200">Rules
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                                ...
                                <td class="border pl-3 pt-1 pb-1 pr-2">
                                    @foreach ($rules as $rule)
                                        @if (str_contains($rule, 'integer'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-blue-800 bg-blue-300 rounded">Integer</span>
                                        @endif
                                        @if (str_contains($rule, 'string'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-green-100 bg-green-500 rounded">String</span>
                                        @endif
                                        @if (str_contains($rule, 'array'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-blue-200 rounded">Array</span>
                                        @endif
                                        @if (str_contains($rule, 'date'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-yellow-400 rounded">Date</span>
                                        @endif
                                        @if (str_contains($rule, 'boolean'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-green-400 rounded">Boolean</span>
                                        @endif
                                        @if (str_contains($rule, 'file') || str_contains($rule, 'image'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-green-400 rounded">File</span>
                                        @endif
                                    @endforeach
                                </td>
                                <td class="border pl-3 pt-1 pb-1 pr-2 bg-gray-100 text-center">
                                    @foreach ($rules as $rule)
                                        @if (str_contains($rule, 'nullable'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none rounded">Nullable</span>
                                        @endif
                                    @endforeach
                                </td>
                                <td class="border pl-3 pt-1 pb-1 pr-2 text-center">
                                    @foreach ($rules as $rule)
                                        @if (str_contains($rule, 'bail'))
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-500 rounded">Bail</span>
                                        @endif
                                    @endforeach
                                </td>
                ...
                >
                    <svg
                        ...
                        class="animate-spin h-4 w-4 rounded-full bg-transparent border-2 border-transparent border-opacity-50 inline pr-2"
                ...
                <div class="grid grid-cols-1 mt-3 pr-2 overflow-auto">
                    <div class="">
                        <div v-if="docs[{{$index}}]['try']">
                            ...
                            <p class="text-xs pb-2 font-medium text-gray-500">Enter your request URL with query
                                params</p>
                            <prism-editor class="my-prism-editor"
                                          ...
                                          v-model="docs[{{$index}}]['url']" :highlight="highlighter"
                                          line-numbers></prism-editor>
                            <br>
                            @if (!in_array('GET', $doc['methods']))
                                <h3 class="font-medium">REQUEST BODY<sup class="text-red-500"> *required</sup></h3>
                                <p class="text-xs pb-2 font-medium text-gray-500">JSON body for the POST|PUT|DELETE
                                    request</p>
                                <prism-editor
                                    class="my-prism-editor"
                    ...
                    <div class="grid grid-cols-2 gap-2"
                         v-if="docs[{{$index}}]['response'] && !docs[{{$index}}]['cancel']">
                        ...
                        <div class="border-r-2">
                            ...
                            <p v-if="!docs[{{$index}}]['queries'].length"
                                ...
                                <tr>
                                    ...
                                    <td class="align-left pl-2 pr-2 break-all">
                                                <span
                                                    class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-yellow-400 rounded">
                                                    <span
                                                        v-text="docs[{{$index}}]['queries'].reduce((total, query) => total + query.time, 0).toFixed(2)"></span>ms
                                                </span>
                                    </td>
                                </tr>
                                </tbody>
                        ...
                        <div class="">
                            ...
                            <div v-for="(query, index) in docs[{{$index}}]['queries']">
                                <p class="mt-2 text-sm font-medium">@{{index+1}}.
                                    <span
                                        class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-yellow-500 rounded mb-1">
                                            @{{query.time}}ms
                                        </span>
                                </p>
                                <prism-editor
...
<script>
    var guessValue = function (attribute, rules) {
        ...
        rules.map(function (rule) {
            ...
            validations.isUuid = rule.match(/uuid/)
        ...
        if (validations.isUuid) {
            validations.value = faker.random.uuid()
        }
